...
| 函数 | 说明 |
|------|------|
| `calculateDynamicCoins()` | 调用博弈论算法计算最优金币分配 |
| `calculateGameTheoryCoins(strategy)` | 核心博弈论算法，支持保守/平衡/激进三种策略 |
| `autoAllocateCoins()` | 执行智能分配并显示分析结果 |
| `suggestCoins(priority, courseId)` | 计算建议金额，避开整数陷阱 |
| `estimateCoursePopularity(courseId)` | 评估课程热度（基于类型、限额、考核难度等） |
| `analyzeTimeSlotCompetition(courseId)` | 分析同时段竞争课程情况 |
| `estimateWinProbability(courseId, coins)` | 估算给定金币投入下的中标概率 |
| `getGameTheoryAnalysis()` | 获取所有选中课程的博弈分析详情 |
| `setGameStrategy(strategy)` | 切换博弈策略（conservative/balanced/aggressive） |
...
5. **避开整数陷阱**（增强版）
   - 避开 50, 100, 150 等常见整数
   - 避开接近整数的值（如 49, 51）
   - 使用素数偏移量 [3, -3, 7, -7, 13, -13, 17, -17, 23, -23]
